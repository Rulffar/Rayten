station-goal-med-bluesky =
    { department-goal-start }[bold]

    Цель смены медицинского отдела:[/bold] синтезировать 60 единиц эфедронекса, обеспечивая своевременное лечение и поддержку здоровья экипажа.

    { department-goal-end }
station-goal-med-Kognizine =
    { department-goal-start }[bold]

    Цель смены медицинского отдела:[/bold] синтезировать 60 единиц когнизина, обеспечивая своевременное лечение и поддержку здоровья экипажа.

    { department-goal-end }
station-goal-Med-nogib =
    { department-goal-start }[bold]

    Цель смены медицинского отдела:[/bold] не допустить безвозвратной смерти ни одного сотрудника в течение как минимум 50 минут с начала смены, демонстрируя высокий профессионализм и оперативность в экстренных ситуациях.

    { department-goal-end }
station-goal-Med-funeral =
    { department-goal-start }[bold]

    Цель смены медицинского отдела:[/bold] провести похороны безвозвратно умершего члена экипажа, уважая традиции и поддерживая моральный дух экипажа.

    { department-goal-end }
station-goal-Med-allsensors =
    { department-goal-start }[bold]

    Цель смены медицинского отдела:[/bold] добиться установки датчиков в режим "координаты" у всего экипажа на станции, повышая уровень безопасности и готовности к чрезвычайным ситуациям.

    { department-goal-end }
station-goal-Med-ImplantTracker =
    { department-goal-start }[bold]

    Цель смены медицинского отдела:[/bold] установить имплант "трекер" 3 сотрудникам, укрепляя систему мониторинга передвижений и безопасности на станции.

    { department-goal-end }
station-goal-Med-ImplantLightning =
    { department-goal-start }[bold]

    Цель смены медицинского отдела:[/bold] установить имплант "свет" 5 сотрудникам, способствуя улучшению условий работы в тёмных зонах станции.

    { department-goal-end }
station-goal-Med-ImplantMemoryShield =
    { department-goal-start }[bold]

    Цель смены медицинского отдела:[/bold] установить имплант "щит памяти" 2 сотрудникам, защищая их сознание от внешних воздействий и потенциальных угроз.

    { department-goal-end }
station-goal-Med-Psyho =
    { department-goal-start }[bold]

    Цель смены медицинского отдела:[/bold] провести психологическое исследование одного из сотрудников станции, удостоверевшись в психологическом здоровье экипажа станции.

    { department-goal-end }
