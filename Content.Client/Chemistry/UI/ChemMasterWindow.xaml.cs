...
namespace Content.Client.Chemistry.UI
{
    ...
    public sealed partial class ChemMasterWindow : FancyWindow
    {
        ...
        private void UpdateMedipenTooltip()
        {
            if (MedipenNumber.Value > 0)
            {
                CreateMedipenButton.TooltipSupplier = (_) =>
                {
                    var tooltipText = GenerateMedipenTooltipText(MedipenNumber.Value, Entity);
                    return new MedipenTooltip(tooltipText);
                };
            }
            else
            {
        ...
        private void UpdateDosageFields(ChemMasterBoundUserInterfaceState castState)
        {
            ...
            MedipenDosage.Value = (int)Math.Min(bufferVolume, castState.MedipenDosageLimit);
        ...
        private string GenerateMedipenTooltipText(int count, EntityUid chemMaster)
        {
            ...
            foreach (var (materialId, perOneAmount) in MedipenRecipe)
            {
                if (!_prototypeManager.TryIndex<MaterialPrototype>(materialId, out var materialProto))
                    continue;
                ...
                var materialName = Loc.GetString(materialProto.Name);
                var unit = Loc.GetString(materialProto.Unit);
                ...
                {
                    var amountText = Loc.GetString("chem-master-window-material-amount", ("required", requiredSheets), ("unit", unit));
                    tooltipText = Loc.GetString("chem-master-window-tooltip-display", ("material", materialName), ("amount", amountText));
                }
