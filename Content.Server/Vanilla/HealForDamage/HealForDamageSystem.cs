...
namespace Content.Server.Vanilla.HealForDamage;
...
public sealed class HealForDamageSystem : EntitySystem
{
    ...
    {
        base.Initialize();
        SubscribeLocalEvent<MobStateComponent, DamageChangedEvent>(HealOnDamage);
    }

    private void HealOnDamage(EntityUid uid, MobStateComponent mobState, DamageChangedEvent args)
    {
        ...
        if (!TryComp<HealForDamageComponent>(origin, out var heal) ||
            !TryComp<MobStateComponent>(uid, out var originMob) ||
            originMob.CurrentState != MobState.Alive)
        {
            return;
        }

        if (!TryComp<TransformComponent>(uid, out var healerXform) ||
            !TryComp<TransformComponent>(origin, out var targetXform))
        {
            return;
        }
